...
# real API server.
SKIP_REAL = False
skip_real = pytest.mark.skipif(SKIP_REAL,
...
def test_old_version_live(library):
    with patch.dict('os.environ', {'ALTERYX_OPEN_SRC_UPDATE_CHECKER': 'TRUE'}):
        with warnings.catch_warnings(record=True) as w:
            ...
            print(library)
            assert len(w) == 1
            assert "{} is out-of-date: installed == 0.0.1, latest == ".format(library) in str(w[-1].message)
    ...
    with patch.dict('os.environ', {'ALTERYX_OPEN_SRC_UPDATE_CHECKER': 'TRUE'}):
        with warnings.catch_warnings(record=True) as w:
            ...
            alteryx_open_src_update_checker.check_version(library=library,
                                                          ...
                                                          headers=headers)
            assert len(w) == 1
            assert "{} is out-of-date: installed == 0.0.1, latest == 0.7.1".format(library) == str(w[-1].message)
